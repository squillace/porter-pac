...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/pac/schema/schema.json.bak"
	schemaPath := "../pkg/pac/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/pac/pac", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the pac schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "pac schema failed")
}
